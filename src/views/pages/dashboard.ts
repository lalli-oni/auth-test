...
export function dashboardPage(options: DashboardPageOptions): string {
  const { user, session, passkeys, message, error } = options;
  ...
  const content = `
      ...
      <div class="dashboard-section">
        ...
        <div class="security-option">
          <div class="security-option-info">
            ...
            <span class="status-badge ${user.totp_enabled ? "status-enabled" : "status-disabled"}">
          ...
          <div class="security-option-actions">
            ...
            `
                : `
              <button onclick="showTotpSetup()" class="btn btn-primary btn-small">Enable</button>
            `
            }
          </div>
        </div>
        ...
        <div class="security-option">
          <div class="security-option-info">
            ...
            <span class="status-badge ${user.email_mfa_enabled ? "status-enabled" : "status-disabled"}">
            ...
            ${passkeys
              ...
              <div class="passkey-item">
                <div class="passkey-info">
                  <strong>${pk.friendly_name || "Unnamed Passkey"}</strong>
                  <span class="passkey-meta">
                    Created: ${new Date(pk.created_at).toLocaleString()}
                    ${pk.backed_up ? " • Backed up" : ""}
                  </span>
                </div>
                <button onclick="deletePasskey('${pk.id}')" class="btn btn-danger btn-small">Remove</button>
              </div>
            `
              )
              .join("")}
    ...
    <div id="totp-setup-modal" class="modal hidden">
      <div class="modal-content">
        <span class="modal-close" onclick="closeTotpSetup()">&times;</span>
  ...
  return layout(content, { title: "Dashboard", user });
}
